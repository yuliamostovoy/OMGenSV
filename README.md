...
First, you'll need to identify structural variation at your locus of interest by examining assembled contigs that align to the locus. Create accurate and complete CMAP representations of each variant, using the reference CMAP and assembled contig CMAPs as templates to be manipulated with the following tools:
...
### Step 2: Decide which CMAPs will be analyzed together and put them into a single CMAP file to serve as the reference for molecule alignment
Some guidelines:
  - Each entry should have a different CMAP ID
...
Pull out the molecules that are local to the region of interest from the Bionano de novo assembly output directory:
...
```
  ...
  where assembly_dirs = list of all the assembly directory names
```
